...
import { Info, HelpCircle, CheckCircle2, AlertCircle, Target, TrendingUp } from "lucide-react";
...
export function MatchExplanation({ score, matchedSkills, missingSkills, explanation }: Props) {
    const getScoreInfo = (s: number) => {
        if (s >= 80) return { label: "Excellent Match", color: "text-emerald-700", bg: "bg-emerald-50", icon: Target };
        if (s >= 60) return { label: "Good Match", color: "text-indigo-700", bg: "bg-indigo-50", icon: TrendingUp };
