...
function renderMarkdown(text: string) {
    // Simple markdown → HTML for job descriptions (server-side, no dependency)
    return text
        .replace(/^## (.+)$/gm, '<h2 class="text-xl font-semibold text-slate-900 mt-6 mb-2">$1</h2>')
        .replace(/^### (.+)$/gm, '<h3 class="text-lg font-medium text-slate-800 mt-4 mb-1">$1</h3>')
        .replace(/\*\*(.+?)\*\*/g, '<strong class="font-semibold text-slate-900">$1</strong>')
        .replace(/^- (.+)$/gm, '<li class="ml-4 text-slate-600 list-disc">$1</li>')
        .replace(/(<li.*<\/li>)/s, '<ul class="mt-2 space-y-1">$1</ul>')
        .replace(/\n\n/g, '</p><p class="mt-3 text-slate-600">')
        .replace(/^([^<].*)$/gm, '<p class="text-slate-600">$1</p>');
}
                ...
                <div className="mt-4 grid gap-6 lg:grid-cols-[1fr_320px]">
                    ...
                    <div className="space-y-6">
                        ...
                        <Card>
                            ...
                            <div
                                className="prose-sm max-w-none text-slate-600 leading-relaxed"
                    ...
                    <div className="space-y-4">
                        <Card className="sticky top-20">
                            ...
                            <Button variant="secondary" className="mt-2 w-full" asChild>
                                <Link href={`/matches?jobId=${job.id}`}>
