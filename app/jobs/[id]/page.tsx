...
import { getJobByIdCached } from "../../../lib/react-cache";
import { Navbar } from "../../../components/Navbar";
import { Card } from "../../../components/Card";
import { JobApplyPanel } from "../../../components/JobApplyPanel";
import { buildMetadata } from "../../../components/SEO";
import { StructuredData, buildJobPostingSchema } from "../../../components/StructuredData";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const job = await getJobByIdCached(params.id);
  if (!job) return buildMetadata({ title: "Job Not Found", noIndex: true });
        ...
        <div className="mt-4 grid gap-6 lg:grid-cols-[1fr_320px]">
          <div className="space-y-6">
            <Card>
              ...
              {job.salary && (
                <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-2.5">
                  <p className="text-sm font-semibold text-emerald-800">${job.salary}</p>
                </div>
              )}

              {deadlineDays !== null && (
                <div className={`mt-2 rounded-xl border px-4 py-2.5 ${deadlineDays <= 7 ? "border-red-200 bg-red-50" : "border-amber-200 bg-amber-50"}`}>
          ...
          <div className="space-y-4">
            <Card className="sticky top-20">
              ...
              <div className="mt-4 border-t border-slate-100 pt-4">
                <p className="mb-2 text-xs font-medium uppercase tracking-wide text-slate-400">Required Skills</p>
                <div className="flex flex-wrap gap-1.5">
                  {job.skills.map((skill) => (
                    <span key={skill} className="rounded-full border border-indigo-100 bg-indigo-50 px-2.5 py-1 text-xs font-medium text-indigo-700">
                      {skill}
                    </span>
                  ))}
                </div>
            ...
            <Card>
              <p className="text-sm font-medium text-slate-700">Browse more {job.category} jobs</p>
              <Link
