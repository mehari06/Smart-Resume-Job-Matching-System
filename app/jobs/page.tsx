...
import { Search, Filter, MapPin, Briefcase, Clock, ChevronLeft, ChevronRight, ExternalLink } from "lucide-react";
...
const CATEGORIES = [
    "All", "Engineering", "Data Science", "AI/ML", "DevOps", "Design",
    "Mobile", "Security", "Data", "Cloud", "Product", "QA", "Management",
    "Content", "Blockchain", "Hardware/IoT", "Networking", "Database",
];
...
export default function JobsPage() {
    ...
    const fetchJobs = useCallback(async (overrides?: { page?: number; search?: string; cat?: string; exp?: string }) => {
        setIsLoading(true);
        const params = new URLSearchParams();
        const s = overrides?.search ?? search;
        const c = overrides?.cat ?? category;
        const e = overrides?.exp ?? experience;
        const p = overrides?.page ?? page;

        if (s) params.set("search", s);
        if (c && c !== "All") params.set("category", c);
        if (e && e !== "All") params.set("experience", e);
    ...
    }, [search, category, experience, page]);
    ...
    const handleCategoryChange = (cat: string) => {
        setCategory(cat);
        fetchJobs({ page: 1, cat });
    };

    const handleExperienceChange = (exp: string) => {
        setExperience(exp);
        fetchJobs({ page: 1, exp });
    };
    ...
    return (
        <div className="main-gradient min-h-screen">
            ...
            <main className="mx-auto w-full max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
                ...
                {hasLoaded && !isLoading && jobs.length === 0 && (
                    <Card className="py-16 text-center">
                        ...
                        <Button className="mt-4" onClick={() => { setSearch(""); setCategory("All"); fetchJobs({ page: 1, search: "", cat: "All" }); }}>
                            Clear filters
                        </Button>
                    </Card>
                )}
